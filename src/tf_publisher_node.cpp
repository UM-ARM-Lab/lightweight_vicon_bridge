    ...
    for (size_t idx_o = 0; idx_o < msg.tracked_objects.size(); idx_o++)
    {
        ...
        for (size_t idx_s = 0; idx_s < current_object.segments.size(); idx_s++)
        {
            ...
            if (!current_segment.occluded)
            {
                ...
                {
                    // const tf2::StamepedTransform current_tf_msg(current_tf, msg.header.stamp, msg.header.frame_id, full_segment_name);
                    current_tf_msg.header.stamp = msg.header.stamp;
                    g_tf_broadcaster->sendTransform(current_tf_msg);
                }
                else
                {
                    // const tf2::StampedTransform current_tf_msg(current_tf, rclcpp::Clock().now(), msg.header.frame_id, full_segment_name);
                    g_tf_broadcaster->sendTransform(current_tf_msg);
                }
            }
        }
...
int main(int argc, char** argv)
{
    ...
    // g_tf_broadcaster = std::unique_ptr<tf2_ros::TransformBroadcaster>(node);
